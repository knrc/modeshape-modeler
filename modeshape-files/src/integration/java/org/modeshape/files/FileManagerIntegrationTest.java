...
public class FileManagerIntegrationTest extends BaseIntegrationTest {
    ...
    public void shouldCreateModelOfSuppliedType() throws Exception {
        fileMgr.modelTypeManager().installSequencers( sequencerUrl( "xml" ) );
        fileMgr.modelTypeManager().installSequencers( sequencerUrl( "sramp" ) );
        fileMgr.modelTypeManager().installSequencers( sequencerUrl( "xsd" ) );
        final String path = upload( "Books.xsd" );
        fileMgr.createModel( path, "Xml Model" );
        final Session session = session();
    ...
    public void shouldFailToCreateModelIfTypeIsInapplicable() throws Exception {
        fileMgr.modelTypeManager().installSequencers( sequencerUrl( "xml" ) );
        fileMgr.createModel( upload( "LICENSE" ), "Xml Model" );
    }
    ...
    private String upload( final String fileName ) throws Exception {
        return fileMgr.upload( new File( getClass().getClassLoader().getResource( fileName ).toURI() ), null );
    }
}
